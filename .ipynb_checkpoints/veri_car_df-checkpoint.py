...
from SR_Dataset import read_MFB, TruncatedTestInputfromMFB, ToTensorTestInput
...
def perform_verification(use_cuda, model, enroll_speaker, test_filename, test_frames, thres):
    enroll_embedding = get_embeddings(use_cuda, enroll_speaker, model, test_frames)
...
def main():
    ...
    for pair in protocol:
        ...
        enroll_speaker = file1.replace('.wav',c.TEST_FEATURE)
        file2 = pair_splited[2]
        test_path = file2.replace('.wav',c.TEST_FEATURE)
        same = int(pair_splited[0])

        thres = c.THRES
        score, result = perform_verification(use_cuda, model, enroll_speaker, test_path, test_frames, thres)
